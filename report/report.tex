...
Target |$\leftarrow$| PickOneState(SymTarget)
    ...
    la pre ric è rispettata perché:
    \begin{itemize}
 ...
 Trace |$\leftarrow$| CreateTrace(SymTrace)
    ...
    quindi Trace' = CreateTrace(SymTrace) rispetta la post di CreateTrace.
    ...
    \begin{numcases}{}
        SymTrace[i] = R \implies Trace[i] \in R; \label{qualcosa1} \\
        \forall i \mbox{ s.t. } 0 \leq i < SymTrace.size - 1, Trace[i+1] \in Post(\{Trace[i]\}). \label{qualcosa2}
    \end{numcases}

    (\ref{qualcosa1}) è rispettata:
    \begin{itemize}
        \item per tutti gli elementi tranne l'ultimo, a seguito della post ricorsiva;
        \item per l'ultimo elemento di Trace = Target $\in$ SymTrace[-1], perché ottenuto tramite PickOneState(SymTrace[-1]).
    \end{itemize}

    (\ref{qualcosa2}) è rispettata $\forall i \mbox{ s.t. } 0 \leq i < SymTrace.size - 2$ a seguito della post ricorsiva, mentre Trace[-1] = target $\in Post(\{Trace[-2]\})$, perché target è ottenuto tramite PickOneState(SymTrace[-1]) e SymTrace[-1] $\subseteq$ Post(SymTrace[-2]).
